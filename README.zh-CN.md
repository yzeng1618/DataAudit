...
只需要 Java 17+。从 [Releases](https://github.com/yzeng1618/DataAudit/releases) 下载 `data-audit-*.jar`（约 150MB——内置了 Iceberg/Trino/JDBC 全部运行时依赖，无需再装任何东西），然后：

```bash
java -jar data-audit.jar demo
```

`demo` 会生成两份带典型差异的 SQLite 样例数据、写好 `task.yaml`、跑一次真实核验，并直接告诉你**哪一行错了、错在哪个字段**。接自己的数据库时：

```bash
java -jar data-audit.jar config init -o task.yaml
# 编辑 task.yaml（见下方配置示例）
java -jar data-audit.jar doctor -f task.yaml
java -jar data-audit.jar check -f task.yaml
```

`doctor` 会默认探测 source/target 连接，问题在执行前就能一行看清。从源码构建见 [docs/development.md](docs/development.md)——那是给贡献者的路径，试用不需要。

> 本文其余命令中的 `data-audit` 均代表 `java -jar data-audit.jar`，可自行为它创建 alias 或 wrapper 脚本。
...
版本发布由 `v*` 标签触发：流水线将 POM 版本对齐到 tag、注入提交溯源并做可复现构建，产物包含完整 CLI、AI wrapper、CycloneDX SBOM、SHA-256 校验文件，并推送容器镜像 `ghcr.io/yzeng1618/data-audit`，见 [Releases](https://github.com/yzeng1618/DataAudit/releases)。
